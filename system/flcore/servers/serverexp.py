import time
from flcore.clients.clientexp import clientEXP
...
class FedEXP(object):
    def __init__(self, args, times, party2loaders, global_train_dl, test_dl):
        ...
        self.set_clients(clientEXP, party2loaders)
    ...
    def aggregate_parameters(self):
        ...
        global_model_w_init = copy.deepcopy(self.global_model.state_dict())
        global_model_w_update = copy.deepcopy(self.global_model.state_dict())
        ...
        for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
            ...
            if temp:
                for key in client_model_w:
                    global_model_w_update[key] = client_model_w[key] * w
                temp = False
            else:
                for key in client_model_w:
                    global_model_w_update[key] += client_model_w[key] * w
        ...
        sqr_avg_delta = 0.0
        for key in global_model_w_update:
            sqr_avg_delta += ((global_model_w_update[key] - global_model_w_init[key])**2).sum()
        ...
        for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
            client_model_w = client_model.state_dict()
            for key in client_model_w:
                avg_sqr_delta += w * ((client_model_w[key] - global_model_w_init[key])**2).sum()

        eta_g = avg_sqr_delta / (2*(sqr_avg_delta + self.eps))
        eta_g = max(1.0, eta_g.item())
        
        print('eta_g at current round: %f' % eta_g)
        
        for key in global_model_w:
            global_model_w[key] = global_model_w_init[key] + eta_g*(global_model_w_update[key] - global_model_w_init[key])
